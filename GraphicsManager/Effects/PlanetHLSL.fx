...
{ 
	...
	float3 ambient		= texCUBE( CubeTexture, normalize(In.Normal));
...
{ 
	...
	//color movable lights	
			...
			float len			= length(lightDir); 								
			nDiffuse		   += lightDiffuse[0] * saturate(dot(nNormal, nLight))* ( (2/len)+ (400/(len*len*len)) );
			
			lightDir			= lightPositions[1] - IN.PPosition;	
			len					= length(lightDir); 	
			lightDir			/= len;
		 	nDiffuse			+= (.20)*lightDiffuse[1] * saturate(dot(sNormal, lightDir));					
		 	
		 	
		 	lightDir			= lightPositions[2] - IN.PPosition;	
			len					= length(lightDir); 	
			lightDir			/= len;
		 	//nDiffuse			+= lightDiffuse[2] * saturate(dot(sNormal, lightDir));			
		 	
		 	/*
		 	lightDir			= lightPositions[3] - IN.PPosition;	
			len					= length(lightDir); 	
			lightDir			/= len;
		 	nDiffuse			+= (.20)*lightDiffuse[3] * saturate(dot(sNormal, lightDir));			
			*/
	...
	for(int i = 0; i < 4; i++)
	{
		sDiffuse += (1-sDiffuse)* saturate(dot(nNormal, normalize(IN.sunLight[i]))*.61);	// saturate(dot(sNormal, sunLightDir[i]))*.6; // 
    }	
