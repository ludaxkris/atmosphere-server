...
v2p RenderSceneVS3( in INPUT IN)
{
	...
	OUT.PPosition = mul(IN.position, g_mWorld);
...
{ 
    ...
    float  height = length(In.PPosition.xyz);	
    float3 color  = tex1D(colorMapTex, (height-18)/3.60);
    	
    //color map
	Output.RGBColor.rgb = color;
    Output.RGBColor.a	= 1;    
...
PS_OUTPUT RenderScenePS3(in v2p IN) 
{ 
    PS_OUTPUT OUT;
	...
	for(int i = 0; i < 3; i++)
		tempDiffuse += saturate(dot(sNormal,sunLightDir[i])*.4);
	
	sDiffuse += tempDiffuse;
	...
	//float4 color  = float4(tex1D(colorMapTex, (height-18)/3.60));
    ...
    float len			= length(lightDir); 
    sDiffuse			+= (1-sDiffuse)*saturate(dot(nNormal, nLight)) * (  (5/len) + (200/(len*len*len)) );
...
technique RenderSceneWithTexture1Light
{
    ...
    {          
        ...
        PixelShader  = compile ps_2_a RenderScenePS3(); 
    }    
}